...
const USER_API_URL = process.env.REACT_APP_USER_API_URL || 'http://localhost:8081';
...
export const createGroup = async (data: CreateGroupRequest, token: string): Promise<GroupResponse> => {
...
export const createUserInfo = async (userId: string, groupId: string, token: string, role: string = 'MEMBER'): Promise<UserInfoResponse> => {
    const data = {
        userId: userId,
        groupId: groupId,
        role: role
